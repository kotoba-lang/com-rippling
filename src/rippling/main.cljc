(ns rippling.main
  "Kotodama WASM entrypoint for the Rippling clean-room actor (L4) — Clojure port.
  ...
  facts (`rippling.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "rippling")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"rippling.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "rippling-compat" :tier tier :entities entities} 200])
...
(defn start! [] :rippling-compat/ready)
